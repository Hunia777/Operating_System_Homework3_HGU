...
{ //START
    ...
    while(condition)
    {
        ...
        if(strcmp(DONE, directory) == 0)
        {
            ...
            condition = 0;
            FILE *fp3;
            if ( (fp3 = fopen("RESULTS.txt", "r")) != NULL)
            {
                char *sending;
                int size;
                int count;
                ...
                size = ftell(fp3);
                sending = malloc(size + 1);
                memset(sending, 0, size + 1);
                fseek(fp3, 0, SEEK_SET);
                count = fread(sending, size, 1, fp3);
                //printf("count = %d\n",count);
                //printf("size = %d\n",size);
                fclose(fp3);
                remove("/home/s21600786/OSHW3/RESULTS.txt");
                ...
                int fd_worker_to_manager = open("worker_to_manager", O_WRONLY|O_SYNC);
                //printf("TEST=======TEST\n");
                //printf("%s\n",sending);
                size_t length = size;
                //printf("len = %d\n", length);
                if (write_bytes(fd_worker_to_manager, &length, sizeof(length)) != sizeof(length))
                    break;
                if (write_bytes(fd_worker_to_manager, sending, length) != length)
                    break;
                
                printf("=========== WORKER SENDING ALL ===========\n");
            }
            else
            {
                break;
                ...
                else //parent
                {
                    ...
                    if (strstr(buf, Asc) != NULL)
                    {
                        ...
                        while (!feof(fd))
                        {
                            ...
                            {
                                int fd2;
                                fd2 = open("RESULTS.txt", O_WRONLY|O_APPEND|O_CREAT, 0666);
                                flock(fd2, LOCK_EX);
                                sprintf(Wrt,"%s:%d:%s", path, line_num, pline);
                                write(fd2, Wrt, strlen(Wrt));
                                flock(fd2, LOCK_UN);
                                close(fd2);
